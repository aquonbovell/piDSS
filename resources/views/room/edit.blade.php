<x-app-layout><x-slot name="header">
  ...
  <div class="max-w-xl mx-auto sm:px-6 lg:px-8 py-12">
    <div class=" text-gray-900 dark:text-gray-100 overflow-hidden rounded-lg dark:shadow-slate-700 p-6">
      <h2 class="py-3 font-black text-3xl text-gray-900 dark:text-gray-100 text-center">Edit piDSS Room</h2>
      <form method="POST" action="{{ route('room.update', $room) }}">
        @csrf
        @method('patch')
        <div>
          <x-input-label for="name" :value="__('Name')" />
          <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{ old('name', $room->name) }}" required autofocus autocomplete="name" />
