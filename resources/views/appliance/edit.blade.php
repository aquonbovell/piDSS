...
<x-slot name="header">
  ...
  <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
    <div class=" text-gray-900 dark:text-gray-100 overflow-hidden rounded-lg dark:shadow-slate-700 p-6">
      <h2 class="py-3 font-black text-3xl text-gray-900 dark:text-gray-100 text-center">Edit piDSS Appliance</h2>
      <form method="POST" action="{{ route('appliance.update' ,$appliance) }}">
        @csrf
        @method('patch')
        <div>
          <x-input-label for="name" :value="__('Name')" />
          <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{ old('name', $appliance->name) }}" required autofocus autocomplete="name" />
          <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>
        <div>
          <x-input-label for="power" :value="__('Power Consumption')" />
          <x-text-input id="power" class="block mt-1 w-full" type="number" name="power" value="{{ old('power', $appliance->power) }}" required autofocus autocomplete="power" />
